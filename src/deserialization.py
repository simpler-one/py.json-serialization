...
def _from_json_obj(source, cls, option, path):
    ...
    if not store.cls.validate_type(type(source)):
    ...
    for key, member in store.members.items():
        ...
        if src is None:
            if member.mandatory:
                raise ValueError(f"Property is mandatory but null or not found: {cur_path}")
            else:
                continue
